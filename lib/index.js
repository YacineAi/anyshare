...
function get(url, token) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const response = await fetch(`https://www.instagram.com/graphql/query/?query_hash=2c4c2e343a8f64c625ba02b2aa12c7f8&variables=%7B%22shortcode%22:%22${vid}%22}`);
      const data = await response.json();
      if (data.data.shortcode_media.is_video == true) {
        const result = {
          instavid: data.data.shortcode_media.video_url
        }
        resolve(result);
      } else { reject(1315); }
    } catch (error) {
