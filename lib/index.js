...
function get(url) {
  if (url.startsWith("https://vm.tiktok.com/")) {
    ...
    return new Promise((resolve, reject) => {
      let uid = url.replace("@", "");
      axios.get('https://instaspells.com/download-instagram-profile-picture/')
      .then((resp) => {
        const $ = cheerio.load(resp.data);
        const token = $('input[name="token"]').attr('value');
        const cookie = resp.headers['set-cookie'].join('');
        fetch("https://instaspells.com/ig-downloader-tools/", {
          "headers": {
            "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
            "cookie": cookie,
            "Referer": "https://instaspells.com/download-instagram-profile-picture/"
          },
          "body": `token=${token}&action=dpDownloader&type=profile&input=https://www.instagram.com/${uid}/`,
          "method": "POST"
        })
        .then(response => response.json())
        .then(data => {
          const result = {
            instavatar: data.images[0].preview_download
          }
          resolve(result);
        }).catch(e => {
          reject({ status: false, message: 'error fetch data', e: e.message })
