...
function get(url) {
  return new Promise((resolve, reject) => {
    ...
    var urling = url.replace("www", "mbasic")
    axios.get(urling)
    .then(({ data }) => {
      var sdMatch = data.match(/video_redirect(.*)FH-R/);
      var clean = sdMatch[0].replace("video_redirect/?src=", "")
      //const sd = parseString(clean);
      const result = {
        facebook: decodeURIComponent(clean)
      }
      resolve(result);
    }).catch(e => {
          reject({ status: false, message: 'error fetch data', e: e.message })
