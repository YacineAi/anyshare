...
async function getFinalUrl(originalUrl) {
  try {
      const response = await axios.get(originalUrl, { maxRedirects: 5 });
...
function get(url, token) {
  return new Promise(async (resolve, reject) => {
    if (url.startsWith("https://vm.tiktok.com/") || url.startsWith("https://vt.tiktok.com/") || url.startsWith("https://www.tiktok.com/") || url.startsWith("https://m.tiktok.com/v/")) {
    ...
    if (url.startsWith("https://www.facebook.com/share/r/") || url.startsWith("https://www.facebook.com/share/v/")) {
      await getFinalUrl(url)
      .then((re) => {
        resolve(get(re, token));
      });
    }
    try {
    ...
    axios.post("https://api.downloadgram.org/media", `url=${url}&v=3&lang=en`, {
      headers: {
        "accept": "*/*",
        "accept-language": "en,ar-DZ;q=0.9,ar;q=0.8",
  ...
  try {
    ...
    const match = htmlString.match(/<source\s+src=\\"[^"\\]*token=([^&"\\]+)/);
    if (!match) throw new Error('Token not found in HTML');
    ...
    const videoUrl = JSON.parse(atob(decoded)).url;
    ...
    reject({ status: false, message: 'error parsing video URL' });
  }
})
.catch(error => {
  ...
  if (error.response?.status == 500) { 
    ...
    reject({ status: false, message: 'error fetch data' });
  }
});
  ...
  .catch(error => {
    ...
    urler(`https://api.pinterest.com/url_shortener/${fx}/redirect/`, function (url) {
      ...
      .then(({ data }) => {
        try {
          var arrayGet = data.match(/"v720P":\s*\{(?:[^{}]*\{[^{}]*\})*[^{}]*"url":\s*"(https:\/\/[^"]+)"/);
          if (arrayGet && arrayGet[1]) {
        const result = {
          pinvid: arrayGet[1]
        };
        resolve(result);
      } else {
        // Fallback to vEXP7 pattern with better error handling
        let arrayGet2 = data.match(/"vEXP7":\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)"url":\s*"(https:\/\/[^"]+)"/);
        
        if (arrayGet2 && arrayGet2[2]) {
          const result = {
            pinvid: arrayGet2[2]
          };
          resolve(result);
        } else {
          // Try alternative video URL patterns
          let fallbackPatterns = [
            /"v720P"[^}]*"url":\s*"(https:\/\/[^"]+)"/,
            /"vHLSV4"[^}]*"url":\s*"(https:\/\/[^"]+)"/,
            /"url":\s*"(https:\/\/v1\.pinimg\.com\/videos\/[^"]+\.mp4)"/
          ];
          ...
          if (!found) {
            // If all regex fails, try JSON parsing as last resort
            try {
              // Extract JSON from script tag
              let jsonMatch = data.match(/__PWS_DATA__\s*=\s*({.*?});/);
              if (jsonMatch) {
                const jsonData = JSON.parse(jsonMatch[1]);
                const videoList = jsonData.props?.initialReduxState?.pins?.objects?.[vid]?.videos?.videoList;
                
                let videoUrl = null;
                if (videoList?.v720P?.url) {
                  videoUrl = videoList.v720P.url;
                } else if (videoList?.vHLSV4?.url) {
                  videoUrl = videoList.vHLSV4.url;
                }
                ...
                if (videoUrl) {
                  ...
                  reject({ status: false, message: 'No video URL found in JSON data' });
                }
              } else {
                reject({ status: false, message: 'No video URL found with any method' });
              }
            } catch (jsonError) {
              reject({ status: false, message: 'Failed to extract video URL', error: jsonError.message });
            }
          }
        }
      }
      } catch (error) {
      reject({ status: false, message: 'Error processing data', error: error.message });
    }
  })
  .catch(e => {
    reject({ status: false, message: 'error fetch data', e: e.message });
